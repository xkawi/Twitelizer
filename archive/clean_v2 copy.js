...
app.get('/train/:cmd', function(req, res){
	...
	if (command == "data"){
		...
		twit.get("statuses/user_timeline", { screen_name: scrname, count: 200}, function(err, reply){
			...
			wstream.end();
			res.send("Total Tweets retrieved: ", reply.length, ". Next is to modify the train data at ", file_name);
		});
		...
		res.send("please specify the command (e.g. /train/data?screen_name or /train/classifier");
	}
});
...
app.get('/classify/:screen_name', function(req, res){
	...
	if (scrname) {
		...
		res.send("missing screen name: /classify/screen_name");
	}
});
...
app.get('/analyse/:screen_name', function(req, res){
	...
	if (scrname){
		...
		fbase.once('value', function(allMessagesSnapshot) {
			...
			res.send(JSON.stringify(classified_tweets));
		});
	} else {
		res.send("missing screen name: /analyse/screen_name");
	}
});
